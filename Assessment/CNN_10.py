...
if best_model:
    ...
    test_proteins = pd.concat([
        positive_test_features.iloc[:, 0],
        negative_test_features.iloc[:, 0]
    ], ignore_index=True)
    ...
    test_results = pd.DataFrame({
        ...
        'Prediction_Probability': test_pred[:, 1]
    })


    result_dir = "../esm2/prediction_results"
    os.makedirs(result_dir, exist_ok=True)
    timestamp = pd.Timestamp.now().strftime("%Y%m%d_%H%M%S")
    prediction_path = os.path.join(result_dir, f"CNN_{timestamp}_predictions.csv")
    test_results.to_csv(prediction_path, index=False)


    fieldnames = ['Model', 'Set', 'Accuracy', 'MCC', 'AUC', 'Sn', 'Sp', 'F1']
    file_exists = os.path.isfile('../Feature Fusion/results.csv')

    with open('../Feature Fusion/results.csv', 'a', newline='') as csvfile:
