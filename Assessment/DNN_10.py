...
for fold, (train_index, val_index) in enumerate(skf.split(features_preprocessed, labels)):
    ...
    if val_mcc > best_mcc:
        ...
        model_save_name = os.path.join(model_save_path, f"best_DNNmodel_fold_{fold + 1}_mcc_{val_sn:.4f}.h5")
        model.save(model_save_name)
        print(f"Saved the best model (Fold {fold + 1}, MCC: {val_sn:.4f}) to {model_save_name}")
