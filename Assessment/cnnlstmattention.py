...
from tensorflow.keras.layers import Input, Conv1D, MaxPooling1D, LSTM, Dense, Dropout, Attention, GlobalAveragePooling1D, concatenate
...
features_preprocessed = np.expand_dims(features_preprocessed, axis=2)
test_features_preprocessed = np.expand_dims(test_features_preprocessed, axis=2)
...
for fold, (train_index, val_index) in enumerate(skf.split(features_preprocessed, labels)):
    ...
    model_save_name = os.path.join(model_save_path, f"best_CNN_LSTM_Attention_model_fold_{fold + 1}.h5")
    checkpoint = ModelCheckpoint(model_save_name, monitor='val_accuracy', save_best_only=True, mode='max')


    history = model.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=10, batch_size=32, callbacks=[checkpoint], verbose=1)
    ...
    if val_sn > best_sn:
        ...
        model_save_name = os.path.join(model_save_path, f"best_CNN_LSTM_Attention_model_fold_{fold + 1}_sn_{val_sn:.4f}.h5")
...
if best_model:
    ...
    test_pred_prob = best_model.predict(test_features_preprocessed)
